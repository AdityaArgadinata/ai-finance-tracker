...
export function TelegramIntegration({ telegram, linkCode, groq, aiStatus }: { telegram: { chatId: number; linkedAt: string } | null; linkCode: { code: string; expiresAt: string } | null; groq: { keyHint: string; model: string; updatedAt: string } | null; aiStatus?: string }) {
  ...
  return (
    <article className="telegram-card">
      ...
      {groq ? <div className="telegram-connected"><i /><div><strong>9Router connected</strong><small>{groq.keyHint} · {groq.model} · Updated {groq.updatedAt}</small><a className="integration-site-link" href="https://9router.com" target="_blank" rel="noreferrer">Visit 9Router <ExternalLink /></a></div><form action={removeGroqApiKey}><button><Unlink /> Revoke</button></form></div> : <div className="telegram-connected integration-pending"><i /><div><strong>Connect 9Router</strong><small>Your key is validated by 9Router and stored encrypted in our database.</small>{aiStatus === "invalid" && <small className="ai-key-error">The API key or model is invalid, or 9Router could not be reached.</small>}<a className="integration-site-link" href="https://9router.com" target="_blank" rel="noreferrer">Visit 9Router <ExternalLink /></a></div><form className="integration-key-form" action={saveGroqApiKey}><input required type="password" name="api_key" autoComplete="off" placeholder="sk-..." /><input required name="model" defaultValue="if/kimi-k2" placeholder="Model ID" /><button><KeyRound /> Save</button></form></div>}
    </article>
  );
}
