...
export function TelegramIntegration({ telegram, linkCode, groq, aiStatus }: { telegram: { chatId: number; linkedAt: string } | null; linkCode: { code: string; expiresAt: string } | null; groq: { keyHint: string; updatedAt: string } | null; aiStatus?: string }) {
  ...
  return (
    <article className="telegram-card">
      ...
      {groq ? <div className="telegram-connected"><i /><div><strong>Groq connected</strong><small>{groq.keyHint} · Updated {groq.updatedAt}</small><a className="integration-site-link" href="https://console.groq.com/home" target="_blank" rel="noreferrer">Visit Groq site <ExternalLink /></a></div><form action={removeGroqApiKey}><button><Unlink /> Revoke</button></form></div> : <div className="telegram-connected integration-pending"><i /><div><strong>Connect Groq</strong><small>Your key is validated by Groq and stored encrypted in our database.</small>{aiStatus === "invalid" && <small className="ai-key-error">The API key is invalid or could not connect to Groq.</small>}<a className="integration-site-link" href="https://console.groq.com/home" target="_blank" rel="noreferrer">Visit Groq site <ExternalLink /></a></div><form className="integration-key-form" action={saveGroqApiKey}><input required type="password" name="api_key" autoComplete="off" placeholder="gsk_..." /><button><KeyRound /> Save key</button></form></div>}
    </article>
  );
}
